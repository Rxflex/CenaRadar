 ...
 * doesn't exist, we return `exists: false` and the card falls back to a
 * compact text placeholder — far cheaper than probing PNG variants and
 * md/lg sizes that almost never differ from the sm JPG.
 */
export async function getImageSize(slug: string): Promise<ImageSize> {
	...
	try {
		...
		return size ? { ...size, exists: true } : DEFAULT;
	} catch {
		return DEFAULT;
	} finally {
